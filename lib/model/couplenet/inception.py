...
class InceptionA(nn.Module):
    ...
    def forward(self, x):
        ...
        print('InceptionA: ', torch.cat(outputs, 1).shape)
...
class InceptionB(nn.Module):
    ...
    def forward(self, x):
        ...
        print('InceptionB: ', torch.cat(outputs, 1).shape)
        return torch.cat(outputs, 1)
...
class InceptionC(nn.Module):
    ...
    def forward(self, x):
        ...
        print('InceptionC: ', torch.cat(outputs, 1).shape)
        return torch.cat(outputs, 1)
...
class InceptionD(nn.Module):
    ...
    def forward(self, x):
        ...
        print('InceptionD: ', torch.cat(outputs, 1).shape)
        return torch.cat(outputs, 1)
...
class InceptionE(nn.Module):
    ...
    def forward(self, x):
        ...
        print('InceptionE: ', torch.cat(outputs, 1).shape)
        return torch.cat(outputs, 1)
...
class inceptionV3(CoupleNet):
    ...
    def _init_modules(self):
        inception = eval('inception_v3({})'.format(self.pretrained))
        ...
        # Freeze weights
        for p in self.RCNN_base[0].parameters(): p.requires_grad = False  # Conv2d_1a_3x3
        for p in self.RCNN_base[1].parameters(): p.requires_grad = False  # Conv2d_2a_3x3
        for p in self.RCNN_base[2].parameters(): p.requires_grad = False  # Conv2d_2b_3x3
        for p in self.RCNN_base[3].parameters(): p.requires_grad = False  # Conv2d_3b_1x1
        for p in self.RCNN_base[4].parameters(): p.requires_grad = False  # Conv2d_4a_3x3
        for p in self.RCNN_base[5].parameters(): p.requires_grad = False  # Mixed_5b
        for p in self.RCNN_base[6].parameters(): p.requires_grad = False  # Mixed_5c
        for p in self.RCNN_base[7].parameters(): p.requires_grad = False  # Mixed_5d
        for p in self.RCNN_base[8].parameters(): p.requires_grad = False  # Mixed_6a
        for p in self.RCNN_base[9].parameters(): p.requires_grad = False  # Mixed_6b
        for p in self.RCNN_base[10].parameters(): p.requires_grad = False  # Mixed_6c
        for p in self.RCNN_base[11].parameters(): p.requires_grad = False  # Mixed_6d
        for p in self.RCNN_base[12].parameters(): p.requires_grad = False  # Mixed_6e
        for p in self.RCNN_base[13].parameters(): p.requires_grad = False  # Mixed_7a
        for p in self.RCNN_base[14].parameters(): p.requires_grad = False  # Mixed_7b
